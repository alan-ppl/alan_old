...
def pad_nones(arg, max_pos_dim):
    ...
    arg = arg.rename(None)
    for i in range(max_pos_dim - pos_dim):
        arg = arg.unsqueeze(-(pos_dim + 1))
...
class CartesianTensor(torch.Tensor):
    ...
    def __torch_function__(self, func, types, args=(), kwargs=None):
        ...
        if func.__name__ in vmap_ops:
            ...
            max_shape = [max(arg.size(name) for arg in tensors(args, kwargs)) for name in unified_names]
            ...
            max_pos_dim = max(sum(name is None for name in arg.names) for arg in tensors(args, kwargs))
