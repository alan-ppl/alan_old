...
with plt.rc_context(bundles.icml2022()):
    ...
    for i in range(len(Ns)):
        for j in range(len(Ms)):
            ...
            with open('results/bus_breakdown_NORMAL/elbo_tmc_new_N{0}_M{1}.json'.format(N,M)) as f:
            ...
            # with open('results/movielens_discrete/elbo_global_N{0}_M{1}.json'.format(N,M)) as f:
            #     results_rws_tmc_new = json.load(f)
            ...
            ax.errorbar(time_rws_tmc_new,elbos_rws_tmc_new, yerr=stds_rws_tmc_new, linewidth=0.55, markersize = 0.75, fmt='-o', c='red', label='MP VI')
    ...
    plt.savefig('charts/chart_bus_breakdown_normal_predll.png'.format(N, M))
    plt.savefig('charts/chart_bus_breakdown_normal_predll.pdf'.format(N, M))
...
with plt.rc_context(bundles.icml2022()):
    ...
    for i in range(len(Ns)):
        for j in range(len(Ms)):
            ...
            # ax[i,j].errorbar(Ks,elbos_rws_tmc, yerr=stds_rws_tmc, linewidth=0.55, markersize = 0.75, fmt='-o', c='orange', label='TMC RWS')
            ax.errorbar(time_rws_tmc_new,elbos_rws_tmc_new, xerr=stds_rws_tmc_new_time, yerr=stds_rws_tmc_new, linewidth=0.55, markersize = 0.75, fmt='-o', c='red', label='MP VI')
            # ax[i,j].errorbar(Ks,elbos_rws_IW, yerr=stds_rws_IW, linewidth=0.55, markersize = 0.75, fmt='-o', c='purple', label='RWS LIW')
    ...
    # fig.tight_layout()
    plt.legend()
    plt.savefig('charts/chart_bus_breakdown_normal_elbo.png'.format(N, M))
    plt.savefig('charts/chart_bus_breakdown_normal_elbo.pdf'.format(N, M))
