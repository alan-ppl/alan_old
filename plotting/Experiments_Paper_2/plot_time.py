...
with plt.rc_context(bundles.icml2022()):
    ...
    for i in range(len(Ns)):
        for j in range(len(Ms)):
            ...
            ax[i,j].errorbar(Ks,elbos_global_K, linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Globally Importance Weighted')
    ...
    # fig.tight_layout()
    plt.legend()
    plt.savefig('charts/chart_time_hier.png'.format(N, M))
    plt.savefig('charts/chart_time_hier.pdf'.format(N, M))
