    ...
    if not QModule:
        ...
        class Q(alan.QModule):
            ...
            def forward(self, tr):
                ...
                tr.sample('sigma_beta', alan.Normal(self.sigma_beta_mean, 0.0001*self.sigma_beta_sigma.exp()))
                tr.sample('mu_beta', alan.Normal(self.mu_beta_mean, 0.0001*self.log_mu_beta_sigma.exp()))
                tr.sample('beta', alan.Normal(self.beta_mu, self.log_beta_sigma.exp()))

                #Borough level
                tr.sample('sigma_alpha', alan.Normal(self.sigma_alpha_mean, 0.25*self.sigma_alpha_sigma.exp()))
                tr.sample('alpha', alan.Normal(self.alpha_mu, self.log_alpha_sigma.exp()))

                #ID level
                tr.sample('log_sigma_phi_psi', alan.Normal(self.log_sigma_phi_psi_mean, 0.0001*self.log_sigma_phi_psi_sigma.exp()))
