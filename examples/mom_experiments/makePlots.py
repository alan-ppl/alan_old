...
def combineALLJsons(experimentName, NUTS=False, dataset_seed=0):
    ...
    for metric in ["MSE", "p_ll", "variance"]:
        ...
        if NUTS:
            fs.append(f"{experimentName}/results/hmc_{experimentName}_MINI100_{metric}_{dataset_seed}.json")
...
def plotAllForSinglePlateCombination(Ks, experimentName, rv, vi_lrs=[0.01,0.001,0.0001], vi_rolling_window=1, vi_eval_k=1, NUTS=False, dataset_seeds=[0]):
    ...
    with plt.rc_context(bundles.icml2022()):
        ...
        for metric in metricToData: 
            ...
            results[metric] = {}
            for method in Ks:
                if not (method == "NUTS" and metric == "elbo"):
        ...
        print(results["vars"]["vi_0.01"]["time"])
        ...
        ax[0,0].errorbar(Ks["global_k"], results["elbo"]["global_k"]["mean"], yerr=results["elbo"]["global_k"]["std_err"], linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Global IS')
        ...
        ax[0,1].errorbar(Ks["global_k"][1:], results["p_ll"]["global_k"]["mean"][1:], yerr=results["p_ll"]["global_k"]["std_err"][1:], linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Global IS')

        ax[0,2].errorbar(Ks["tmc_new"][1:],  results["vars"]["tmc_new"]["mean"][1:],  yerr=0,  linewidth=0.55, markersize = 0.75, fmt='-o', c='red', label='MP IS')
        ax[0,2].errorbar(Ks["global_k"][1:], results["vars"]["global_k"]["mean"][1:], yerr=0, linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Global IS')

        ax[0,3].errorbar(Ks["tmc_new"][1:],  results["MSE"]["tmc_new"]["mean"][1:],  yerr=0,  linewidth=0.55, markersize = 0.75, fmt='-o', c='red', label='MP IS')
        ax[0,3].errorbar(Ks["global_k"][1:], results["MSE"]["global_k"]["mean"][1:], yerr=0, linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Global IS')
        ...
        ax[1,0].errorbar(results["elbo"]["global_k"]["time"], results["elbo"]["global_k"]["mean"], yerr=results["elbo"]["global_k"]["std_err"], linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Global IS')
        for alpha, lr in enumerate(vi_lrs[::-1]):
            # if not(experimentName == "bus_breakdown" and lr in [0.1, 0.01]):
            ax[1,0].plot(results["elbo"][f"vi_{lr}"]["time"][:101], results["elbo"][f"vi_{lr}"]["mean"][:101], linewidth=0.55, markersize = 0.75, label="VI lr="+str(lr), c=(0,0,1,1/(alpha+1)))
        ...
        ax[1,1].errorbar(results["p_ll"]["global_k"]["time"][1:], results["p_ll"]["global_k"]["mean"][1:], yerr=results["p_ll"]["global_k"]["std_err"][1:], linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Global IS')
        for alpha, lr in enumerate(vi_lrs[::-1]):
            ax[1,1].plot(results["p_ll"][f"vi_{lr}"]["time"][:101], results["p_ll"][f"vi_{lr}"]["mean"][:101], linewidth=0.55, markersize = 0.75, label="VI lr="+str(lr), c=(0,0,1,1/(alpha+1)))
        if NUTS:
            ax[1,1].plot(results["p_ll"]["NUTS"]["time"], results["p_ll"]["NUTS"]["mean"], linewidth=0.55, markersize = 0.75, label="NUTS", c='black')
        ...
        ax[1,2].errorbar(results["vars"]["global_k"]["time"][1:], results["vars"]["global_k"]["mean"][1:], yerr=0, linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Global IS')
        for alpha, lr in enumerate(vi_lrs[::-1]):
            ax[1,2].plot(results["vars"][f"vi_{lr}"]["time"][:101], results["vars"][f"vi_{lr}"]["mean"][:101], linewidth=0.55, markersize = 0.75, label="VI lr="+str(lr), c=(0,0,1,1/(alpha+1)))
        if NUTS:
            ax[1,2].plot(results["vars"]["NUTS"]["time"], results["vars"]["NUTS"]["mean"], linewidth=0.55, markersize = 0.75, label="NUTS", c='black')
        ...
        ax[1,3].errorbar(results["MSE"]["global_k"]["time"][1:], results["MSE"]["global_k"]["mean"][1:], yerr=0, linewidth=0.55, markersize = 0.75, fmt='-o', c='green', label='Global IS')
        for alpha, lr in enumerate(vi_lrs[::-1]):
            ax[1,3].plot(results["MSE"][f"vi_{lr}"]["time"], results["MSE"][f"vi_{lr}"]["mean"], linewidth=0.55, markersize = 0.75, label="VI lr="+str(lr), c=(0,0,1,1/(alpha+1)))
        if NUTS:
            ax[1,3].plot(results["MSE"]["NUTS"]["time"], results["MSE"]["NUTS"]["mean"], linewidth=0.55, markersize = 0.75, label="NUTS", c='black')
...
# plotAllForSinglePlateCombination(Ks, "bus_breakdown", "alpha", vi_lrs=vi_lrs, NUTS=False)
