...
for dataset_seed in dataset_seeds:
    ...
    for useData in [True, False]:
        ...
        P, Q, data, covariates, all_data, all_covariates, _ = generate_model(0,0, device, dataset_seed=dataset_seed)
                    ...
                    try:
                        if useData:
                            ...
                            if k in Ks["tmc_new"]:
                                ...
                                elbos["tmc_new"][k].append(model.elbo_tmc_new(k, reparam=False).item())
                            ...
                            elbos["global_k"][k].append(model.elbo_global(k, reparam=False).item())
        ...
        if useData:
            file = f'{resultsFolder}/occupancy{results_tag}_elbo_{dataset_seed}.json'
            with open(file, 'w') as f:
                json.dump(elbos, f, indent=4)

            file = f'{resultsFolder}/occupancy{results_tag}_p_ll_{dataset_seed}.json'
            with open(file, 'w') as f:
                json.dump(p_lls, f, indent=4)

            file = f'{resultsFolder}/occupancy{results_tag}_variance_{dataset_seed}.json'
            with open(file, 'w') as f:
                json.dump(expectations, f, indent=4)
        else:
            file = f'{resultsFolder}/occupancy{results_tag}_MSE_{dataset_seed}.json'
            with open(file, 'w') as f:
                json.dump(expectations, f, indent=4)
