...
def P(tr):
    ...
    tr['c'] = tpp.Normal(tr['b'], 1, sample_shape=1, sample_names='plate_1')
    tr['d'] = tpp.Normal(tr['c'], 1, sample_shape=1, sample_names='plate_2')
...
class Q(nn.Module):
    def __init__(self):
        ...
        self.m_c = nn.Parameter(t.zeros((1,), names=('plate_1',)))
        self.m_d = nn.Parameter(t.zeros((1, 1), names=('plate_2','plate_1')))

        self.log_s_a = nn.Parameter(t.zeros(()))
        self.log_s_b = nn.Parameter(t.zeros(()))
        self.log_s_c = nn.Parameter(t.zeros((1,), names=('plate_1',)))
        self.log_s_d = nn.Parameter(t.zeros((1, 1), names=('plate_2','plate_1')))
...
mean_d = 1/cov_d * (1/5 * N * data['obs'].rename(None).mean(axis=0))
print(mean_d)
...
mean_c = 1/cov_c * (1/5 * N * data['obs'].rename(None).mean(axis=0))
print(mean_c)
...
mean_b = 1/cov_b * (1/5 * N * data['obs'].rename(None).mean(axis=0))
print(mean_b)
...
mean_a = 1/cov_a * (1/5 * N * data['obs'].rename(None).mean(axis=0))
print(mean_a)
