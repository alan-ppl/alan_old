...
def generate_model(N,M, run=0, use_data=True):
    ...
    class Q(alan.AlanModule):
        def __init__(self):
            super().__init__()
            #mu_z
            self.mu = alan.AMMP_ISNormal(sample_shape=(d_z,))
            #psi_z
            self.psi_z = alan.AMMP_ISNormal(sample_shape=(d_z,))

            #z
            self.z = alan.AMMP_ISNormal({'plate_1': M},sample_shape=(d_z,))
...
if __name__ == "__main__":
    ...
    P, Q, data, covariates, all_data, all_covariates, sizes = generate_model(5,300, run=0, use_data=False)
    ...
    for j in range(2000):
        ...
        elbo = sample.elbo()
        model.ammpis_update(0.15, sample)
