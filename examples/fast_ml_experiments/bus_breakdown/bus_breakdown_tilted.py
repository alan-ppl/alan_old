    ...
    if ML == 1:
        class Q(alan.AlanModule):
            def __init__(self):
                super().__init__()
                #sigma_beta
                self.sigma_beta = alan.TiltedNormal()
                #mu_beta
                self.mu_beta = alan.TiltedNormal()
                #beta
                self.beta = alan.TiltedNormal({'plate_Year': M})
                #sigma_alpha
                self.sigma_alpha = alan.TiltedNormal({'plate_Borough': J})
                #alpha
            ...
            def forward(self, tr, run_type, bus_company_name):
                ...
                tr('sigma_beta', self.sigma_beta(0,1))
                tr('mu_beta', self.mu_beta(0,1))
                tr('beta', self.beta(tr['mu_beta'], 1))

                #Borough level
                tr('sigma_alpha', self.sigma_alpha(0,1))
                tr('alpha', self.alpha(tr['beta'], 1))

                #ID level
                tr('log_sigma_phi_psi', self.log_sigma_phi_psi(0,1))
                tr('psi', self.psi(0, 1))
                tr('phi', self.phi(0, 1))
                # tr('theta', self.theta())
    elif ML == 2:
