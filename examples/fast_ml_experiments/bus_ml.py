...
for use_data in [True]:
    ...
    for K in [5,10]:
        ...
        fig_times, ax_times = plt.subplots(1,3, figsize=(7.0, 7.0))
        fig_iters, ax_iters = plt.subplots(1,3, figsize=(7.0, 7.0))
        #ML with decay LR
        print('ML Decay')
        for j in range(len(ml_lrs)):
            ...
            try:
                for i in range(T):
                    ...
                    for k,v in sample.weights().items():
                        non_zero_weight += t.count_nonzero(v[1])
                        num_weights += v[1].numel()
                    
                    non_zero_weights.append(non_zero_weight/num_weights)
                ...
                times.append(np.nan)
                non_zero_weights.append(np.nan)
            ...
            ax[0].plot(np.cumsum(times)[::25], n_mean(z_means,25).squeeze(0), color=decay_colours[j], label=f'ML decay: {exp_lr}')
            ax[0].axhline(z_mean)
            ax[1].axhline(z_scale_mean)
            ax[1].plot(np.cumsum(times)[::25], n_mean(z_scale_means,25).squeeze(0), color=decay_colours[j])
            ax[2].plot(np.cumsum(times)[::25], n_mean(elbos,25).squeeze(0), color=decay_colours[j])
            ax[3].plot(np.cumsum(times)[::25], n_mean(pred_lls,25).squeeze(0), color=decay_colours[j])

            ax_times[0].plot(np.cumsum(times)[::25], n_mean(elbos,25).squeeze(0), color=decay_colours[j], label=f'ML decay: {exp_lr}')
            ax_times[1].plot(np.cumsum(times)[::25], n_mean(pred_lls,25).squeeze(0), color=decay_colours[j])
            ax_times[2].plot(np.cumsum(times), scales.squeeze(0), color=decay_colours[j])
            ax_times[3].plot(np.cumsum(times), non_zero_weights.squeeze(0), color=decay_colours[j])

            ax_iters[0].plot(np.arange(T)[::25], n_mean(elbos,25).squeeze(0), color=decay_colours[j], label=f'ML decay: {exp_lr}')
            ax_iters[1].plot(np.arange(T)[::25], n_mean(pred_lls,25).squeeze(0), color=decay_colours[j])
            ax_iters[2].plot(np.arange(T), scales.squeeze(0), color=decay_colours[j])
            ax_iters[3].plot(np.arange(T), non_zero_weights.squeeze(0), color=decay_colours[j])
        ...
        ax_times[0].plot(np.cumsum(times)[::25], n_mean(elbos,25).squeeze(0), color=ml_colours[0], label=f'ML lr: {lr}')
        ax_times[1].plot(np.cumsum(times)[::25], n_mean(pred_lls,25).squeeze(0), color=ml_colours[0])
        ax_times[2].plot(np.cumsum(times), scales.squeeze(0), color=ml_colours[0])
        ax_times[3].plot(np.cumsum(times), non_zero_weights.squeeze(0), color=ml_colours[0])

        ax_iters[0].plot(np.arange(T)[::25], n_mean(elbos,25).squeeze(0), color=ml_colours[0], label=f'ML lr: {lr}')
        ax_iters[1].plot(np.arange(T)[::25], n_mean(pred_lls,25).squeeze(0), color=ml_colours[0])
        ax_iters[2].plot(np.arange(T), scales.squeeze(0), color=ml_colours[0])
        ax_iters[3].plot(np.arange(T), non_zero_weights.squeeze(0), color=ml_colours[0])
        ...
        for j in range(len(vi_lrs)):
            ...
            ax_times[0].plot(np.cumsum(times)[::25], n_mean(elbos,25).squeeze(0), color=vi_colours[j], label=f'VI lr: {lr}')
            ax_times[1].plot(np.cumsum(times)[::25], n_mean(pred_lls,25).squeeze(0), color=vi_colours[j])

            ax_iters[0].plot(np.arange(T)[::25], n_mean(elbos,25).squeeze(0), color=vi_colours[j], label=f'VI lr: {lr}')
            ax_iters[1].plot(np.arange(T)[::25], n_mean(pred_lls,25).squeeze(0), color=vi_colours[j])
        ...
        ax_times[0].set_title(f'K: {K}, Smoothed, Use data: {use_data}')
        ax_times[0].set_ylabel('ELBO')
        ax_times[1].set_ylabel('Predictive LL')
        ax_times[2].set_ylabel('Approx posterior Scale')
        ax_times[1].set_xlabel('Time')
        fig_times.legend(loc='upper right')


        fig_iters.tight_layout()
        fig_iters.savefig(f'figures/bus_test_data_{K}_{use_data}_elbo_iters.png')


        ax_iters[0].set_title(f'K: {K}, Smoothed, Use data: {use_data}')
        ax_iters[0].set_ylabel('ELBO')
        ax_iters[1].set_ylabel('Predictive LL')
        ax_iters[2].set_ylabel('Approx posterior Scale')
        ax_iters[1].set_xlabel('Time')
        fig_iters.legend(loc='upper right')


        fig_iters.tight_layout()
        fig_iters.savefig(f'figures/bus_test_data_{K}_{use_data}_elbo.png')import torch as t
import torch.nn as nn
import pickle
import alan
import alan.postproc as pp
...
for use_data in [True]:
    ...
    for K in [5,10]:
        ...
        fig_iters.tight_layout()
        fig_iters.savefig(f'figures/bus_test_data_{K}_{use_data}_elbo.png')