...
for use_data in [True]:
    ...
    for K in [5,10]:
        ...
        fig_scales, ax_scales = plt.subplots(1,8, figsize=(20.0, 6.0))
        fig_weights, ax_weights = plt.subplots(1,8, figsize=(20.0, 6.0))
        #ML with decay LR
        print('ML Decay')
        for j in range(len(ml_lrs)):
            ...
            try:
                for i in range(T):
                    ...
                    for k,v in sample.weights().items():
                        ...
                        weights[k].append(t.count_nonzero(v[1].rename(None)))

                        non_zero_weight += t.count_nonzero(v[1].rename(None))
            ...
            elbos = n_mean(elbos,1).squeeze(0)
            pred_lls = n_mean(pred_lls,1).squeeze(0)
            ...
            ax_iters[0].plot(elbos, color=decay_colours[j], label=f'ML decay: {exp_lr}')
            ax_iters[1].plot(pred_lls, color=decay_colours[j])
            ax_iters[2].plot(non_zero_weights, color=decay_colours[j])


            for i, (k,v) in enumerate(scales.items()):
                v = np.expand_dims(np.array(v), axis=0)
                if i == 0:
                    ax_scales[i].plot(v.squeeze(0), color=decay_colours[j], label=f'ML decay: {exp_lr}')
                else:
                    ax_scales[i].plot(v.squeeze(0), color=decay_colours[j])
                    
                ax_scales[i].set_title(k)


            for i, (k,v) in enumerate(weights.items()):
                v = np.expand_dims(np.array(v), axis=0)
                if i == 0:
                    ax_weights[i].plot(v.squeeze(0), color=decay_colours[j], label=f'ML decay: {exp_lr}')
                else:
                    ax_weights[i].plot(v.squeeze(0), color=decay_colours[j])
                    
                ax_weights[i].set_title(k)
        ...
        ax_iters[0].plot(elbos, color=ml_colours[0], label=f'ML lr: {lr}')
        ax_iters[1].plot(pred_lls, color=ml_colours[0])
        ax_iters[2].plot(non_zero_weights, color=ml_colours[0])

        for i, (k,v) in enumerate(scales.items()):
            v = np.expand_dims(np.array(v), axis=0)
            if i == 0:
                ax_scales[i].plot(v.squeeze(0), color=ml_colours[0], label=f'ML lr: {lr}')
            else:
                ax_scales[i].plot(v.squeeze(0), color=ml_colours[0])
        ...
        for i, (k,v) in enumerate(weights.items()):
            v = np.expand_dims(np.array(v), axis=0)
            if i == 0:
                ax_weights[i].plot(v.squeeze(0), color=ml_colours[0], label=f'ML lr: {lr}')
            else:
                ax_weights[i].plot(v.squeeze(0), color=ml_colours[0])
        ...
        for j in range(len(vi_lrs)):
            ...
            ax_iters[0].plot(n_mean(elbos,1).squeeze(0), color=vi_colours[j], label=f'VI lr: {lr}')
            ax_iters[1].plot(n_mean(pred_lls,1).squeeze(0), color=vi_colours[j])
            ax_iters[2].plot(n_mean(non_zero_weights,1).squeeze(0), color=vi_colours[j])

            for i, (k,v) in enumerate(scales.items()):
                v = np.expand_dims(np.array(v), axis=0)
                if i == 0:
                    ax_scales[i].plot(v.squeeze(0), color=vi_colours[j], label=f'VI lr: {lr}')
                else:
                    ax_scales[i].plot(v.squeeze(0), color=vi_colours[j])
                ax_scales[i].set_title(k)
            ...
            for i, (k,v) in enumerate(weights.items()):
                v = np.expand_dims(np.array(v), axis=0)
                if i == 0:
                    ax_weights[i].plot(v.squeeze(0), color=vi_colours[j], label=f'VI lr: {lr}')
                else:
                    ax_weights[i].plot(v.squeeze(0), color=vi_colours[j])
        ...
        fig_times.savefig(f'figures/bus_test_data_{K}_{use_data}_elbo.png')
        ...
        fig_scales.savefig(f'figures/bus_test_data_{K}_{use_data}_scales.png')


        fig_weights.legend(loc='upper right')
        fig_weights.tight_layout()
        fig_weights.savefig(f'figures/bus_test_data_{K}_{use_data}_weights.png')