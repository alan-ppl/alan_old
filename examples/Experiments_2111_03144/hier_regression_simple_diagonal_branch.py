...
def P(tr):
  ...
  Heirarchical Model
  '''
  tr['theta'] = alan.Normal(theta_mean, theta_sigma)
  tr['z'] = alan.Normal(tr['theta'], z_sigma, sample_dim=plate_1)
...
class Q(alan.Q_module):
    def __init__(self):
        ...
        self.reg_param("log_theta_s", t.randn((theta_size,)))
    ...
    def forward(self, tr):
        ...
        tr['theta'] = alan.Normal(self.theta_mu, self.log_theta_s.exp())
        tr['z'] = alan.Normal(self.z_mu , self.log_z_s.exp())
...
np.save('Diagonal_K{0}_N{1}.npy'.format(K, N),elbos)
