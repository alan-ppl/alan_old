...
def generate_model(N,M,local,device):
    ...
    def P(tr):
        ...
        print(tr['psi_int'])
        tr.sample('obs', alan.Normal(tr['omega'] + tr['psi_int']*tr['basement'], tr['sigma_obs'] + 1))
    ...
    class Q(alan.QModule):
        def __init__(self):
            super().__init__()
            #statevariance
            self.sigma_beta = nn.Parameter(t.randn(10))
            #statemean
            ...
            #countyvariance
            self.sigma_alpha = nn.Parameter(t.randn((M,10),names=('plate_state',None)))
            #w
            self.gamma = nn.Parameter(t.randn((J,10),names=('plate_county',None)))
            self.w_gamma = nn.Parameter(t.randn((J,),names=('plate_county',)))
            #zipmean
            ...
            #zipvariance
            self.sigma_omega = nn.Parameter(t.randn((J,10),names=('plate_county',None)))
            #readingmean
            ...
            #readingvariance
            self.sigma_obs = nn.Parameter(t.randn((I,10),names=('plate_zipcode',None)))
            #b
