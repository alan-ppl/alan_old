...
T = 2000
lrs = {"natural_rws": [0.01, 0.03, 0.1, 0.3],
       "natural_rws_difference": [0.0001, 0.0005, 0.001, 0.005],
       "natural_rws_standardised": [0.01, 0.03, 0.1, 0.3]}
...
for num_latents, K, VAR_SIZE_STR in product(Ns, Ks, var_sizes):
    ...
    for i, fn in enumerate(rws_variants):
        for j, lr in enumerate(lrs[fn.__name__]):
            rws_results[(i,j)] = fn(T, post_params, init, lr, K)
    ...
    for use_time in [True, False]:
        ...
        for i, fn in enumerate(rws_variants):
            for j, lr in enumerate(lrs[fn.__name__]):
                m_q, l_one_iters, entropies, times = fn(T, post_params, init, lr, K)
                ...
                l0, = ax[0].plot(x_axis(times, use_time), mean_errs, c=colors[i], alpha = (j+1)/len(lrs[fn.__name__]), label=f'{fn.__name__}')

                l1, = ax[1].plot(x_axis(times, use_time), var_errs, c=colors[i], alpha = (j+1)/len(lrs[fn.__name__]), label=f'{fn.__name__}')

                l2, = ax[2].plot(x_axis(times[3:], use_time), [lt for lt in l_one_iters][2:], c=colors[i], alpha = (j+1)/len(lrs[fn.__name__]), label=f'{fn.__name__}')

                l3, = ax[3].plot(x_axis(times, use_time), entropies, c=colors[i], alpha = (j+1)/len(lrs[fn.__name__]), label=f'{fn.__name__}')

                l4, = ax[4].plot(x_axis(times, use_time), [(fit_approx_post(m)[:,1] / post_params[:,1]).mean() for m in m_q], c=colors[i], alpha = (j+1)/len(lrs[fn.__name__]), label=f'{fn.__name__}')
        ...
        hmc_line, = ax[0].plot(x_axis(hmc_times, use_time), hmc_mean_errs, c='black', label='HMC')
        final_lines.append(hmc_line)
        ax[1].plot(x_axis(hmc_times, use_time), hmc_var_errs, c='black', label='HMC')
        ...
        plt.tight_layout()  
        plt.savefig(f"figures/rws_lrs/N{num_latents}_K{K}_{VAR_SIZE_STR}{'_TIME' if use_time else ''}.png")
        plt.close()
