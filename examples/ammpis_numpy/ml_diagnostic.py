...
diff_ml1_natural = 0
diff_ml2_natural = 0
diff_ml1toy_natural = 0
diff_ml2toy_natural = 0
for i in range(T):
    ...
    if np.abs(l_one_iters[i] - elbos_ml2[i]) > 0.001:
        diff_ml2_natural += 1
    if np.abs(l_one_iters[i] - l_one_iters_ml1[i]) > 0.001:
        diff_ml1toy_natural += 1
    if np.abs(l_one_iters[i] - l_one_iters_ml2[i]) > 0.001:
        diff_ml2toy_natural += 1
...
print(f'Number of iterations where ML1 Toy and Natural RWS differ: {diff_ml1toy_natural}')
print(f'Number of iterations where ML2 Toy and Natural RWS differ: {diff_ml2toy_natural}')
