...
class P(nn.Module):
    ...
    def forward(self, tr):
        ...
        tr.sample('mu_z', alan.Normal(t.zeros((d_z,)).to(device), t.ones((d_z,)).to(device)), group='group_1')
        tr.sample('psi_z', alan.Categorical(t.tensor([0.1,0.5,0.4,0.05,0.05]).to(device)), group='group_1')
