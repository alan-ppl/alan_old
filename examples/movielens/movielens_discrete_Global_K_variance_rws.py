...
class P(nn.Module):
    ...
    def forward(self, tr):
        ...
        tr['mu_z'] = tpp.Normal(t.zeros((2,d_z)).to(device), t.ones((2,d_z)).to(device), group='group1')
        tr['psi_z'] = tpp.Categorical(t.tensor([0.1,0.5,0.4,0.05,0.05]).to(device), group='group1')
