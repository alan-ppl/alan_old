...
fp2 = FontProperties(
    fname=os.path.join(
        os.path.dirname(__file__), "../../fonts/Font Awesome 5 Free-Solid-900.otf"
    )
)
...
class PreprocessedData(object):
    ...
    def __init__(
        ...
        :param ActiveCMs: NPI Intervention data np.ndarray binary array. Shape is (nRs, nCMs, nDs)
    ...
    def remove_regions_min_deaths(self, min_num_deaths=100):
        ...
        for indx, r in enumerate(self.Rs):
            if self.Deaths.data[indx, -1] < min_num_deaths:
                print(
                    f"Region {r} removed since it has {self.Deaths[indx, -1]} deaths on the last day"
                )
    ...
    def conditional_activation_plot(
        self, cm_plot_style, newfig=True, skip_yticks=False
    ):
        """
        ...
        x_r = x_max - x_min
        for i, (ticklabel, tickloc) in enumerate(
            zip(ax.get_yticklabels(), ax.get_yticks())
        ):
            ticklabel.set_color(cm_plot_style[i][1])
            plt.text(
                -0.16 * x_r,
    ...
    def cumulative_days_plot(self, cm_plot_style, newfig=True, skip_yticks=False):
        ...
        days_active = np.sum(
            np.sum(self.ActiveCMs * np.repeat(mask, nCMs, axis=1), axis=0), axis=1
        )
        plt.barh(
            -np.arange(nCMs),
            days_active,
            color=[0.7522446028276593, 0.5089037847613617, 0.6733963201089419],
        )
