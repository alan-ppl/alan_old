...
def preprocess_data(
    data_path,
    last_day=None,
    schools_unis="two_separate",
    drop_features=None,
    min_confirmed=100,
    min_deaths=10,
    smoothing=1,
    mask_zero_deaths=False,
    ...
    if smoothing != 1:
        ...
        SmoothedNewCases = np.around(
            ss.convolve2d(
                NewCases,
                1 / smoothing * np.ones(shape=(1, smoothing)),
                boundary="symm",
                mode="same",
            )
        )
    ...
    if schools_unis == "two_xor":
        ...
        ActiveCMs_final[:, school_index, :] = np.logical_and(
            ActiveCMs[:, university_index, :], ActiveCMs[:, school_index, :]
        )
        ActiveCMs_final[:, university_index, :] = np.logical_xor(
            ActiveCMs[:, university_index, :], ActiveCMs[:, school_index, :]
        )
