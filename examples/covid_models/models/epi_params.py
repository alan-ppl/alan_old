...
Authors: Gavin Leech and Charlie Rogers Smith
Source: https://github.com/g-leech/masks_v_mandates/blob/main/epimodel/pymc3_models/epi_params.py
...
def bootstrapped_negbinom_values(
    delays, n_bootstrap=250, n_rvs=int(1e7), truncation=64, filter_disp_outliers=True
):
    """
    ...
    for seed in tqdm(range(n_bootstrap)):
        ...
        disps[seed] = (
            (np.var(samples) - np.mean(samples)) / (np.mean(samples) ** 2)
        ) ** -1
...
class EpidemiologicalParameters:
    ...
    def generate_dist_samples(self, dist, nRVs, with_noise):
        ...
        mean = np.random.normal(
            loc=dist["mean_mean"], scale=dist["mean_sd"] * with_noise
        )
        if dist["dist"] == "gamma":
        ...
        if dist["dist"] == "gamma_cov":
            cov = np.random.normal(
                loc=dist["cov_mean"], scale=dist["cov_sd"] * with_noise
            )
            sd = cov * mean
            k = mean ** 2 / sd ** 2
            theta = sd ** 2 / mean
            ...
            disp = np.random.normal(
                loc=dist["disp_mean"], scale=dist["disp_sd"] * with_noise
            )
            p = disp / (disp + mean)
    ...
    def generate_reporting_and_fatality_delays(
        self, nRVs=int(1e7), with_noise=True, max_reporting=32, max_fatality=48
    ):
        """
        ...
        """
        incubation_period_samples = self.generate_dist_samples(
            self.incubation_period, nRVs, with_noise
        )
        reporting_samples = self.generate_dist_samples(
            self.infection_to_reporting_delay, nRVs, with_noise
        )
        fatality_samples = self.generate_dist_samples(
            self.infection_to_fatality_delay, nRVs, with_noise
        )

        print(
            f"Raw: reporting delay mean {np.mean(incubation_period_samples + reporting_samples)}"
        )
        print(
            f"Raw: fatality delay mean {np.mean(incubation_period_samples + fatality_samples)}"
        )
        reporting_delay = self.discretise_samples(
            incubation_period_samples + reporting_samples, max_reporting
        )
        fatality_delay = self.discretise_samples(
            incubation_period_samples + fatality_samples, max_fatality
        )
        print(
            f"Generated Reporting Delay: {self.generate_pmf_statistics_str(reporting_delay)}"
        )
        print(
            f"Generated Fatality Delay: {self.generate_pmf_statistics_str(fatality_delay)}"
        )
